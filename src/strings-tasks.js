...
function removeFirstOccurrences(str, value) {
  ...
  const { length } = value;
  return index >= 0 ? str.slice(0, index) + str.slice(index + length) : str;
}
...
function isPalindrome(str) {
  const MARKS = [',', '.', '?', '!', ':', ';', ' '];
  const noSpaces = str.split('').map((char) => {
    const lower = char.toLowerCase();
    return MARKS.includes(char) ? '' : lower;
  });
  return noSpaces.slice().reverse().join('') === noSpaces.join('');
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce(
      (largest, word) => (word.length > largest.length ? word : largest),
      ''
    );
}
...
function extractNameFromTemplate(value) {
  ...
  const [_, firstName, lastName] = value.split(' ');
